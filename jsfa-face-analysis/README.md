...
スコアの重み・換算を、施術者の評価に合わせて調整できます。**すべて端末内で完結し、顔写真は保存されません**（保存されるのは各指標の数値と正解スコアのみ）。

1. URL の末尾に `?calib=1` を付けて開く（例：`…vercel.app/?calib=1`）
2. 通常どおり Before/After を解析 → 結果画面下部に「🔧 校正モード」パネルが表示される
3. 施術者がその結果の**正解スコア（0〜100）**を入力し「記録」
4. これを **20件以上**を目安に蓄積
5. 「係数を再計算」→ リッジ回帰で重み・換算を推定し、当てはまり（R²）とともに反映
6. 以降の解析は校正済み係数で計算される（「既定に戻す」で元に戻せる／「CSV書き出し」で生データを取得可）

> 少数データでは過学習しやすいため、R² が高すぎる／係数が極端な場合はサンプルを増やしてください。データが偏らないよう、改善が大きい例・小さい例をバランス良く含めるのがコツです。
